...
<html>
  <head>
    <meta charset="utf-8">
    <title>Prenotazioni | Biblioteca online</title>
  ...
  <body>
    ...
    <div class="container-fluid first-section center">
      <div class="layer-band center">
        <h2 class="display-4">
        <c:if test="${titolo!=null && titolo!=''}">
         ${titolo} ${prenotazioni[0].utente.nome} ${prenotazioni[0].utente.cognome}
        </c:if>
        <c:if test="${titolo==null || titolo==''}">
          ...
          <table class="table">
            ...
            <c:forEach var="prenotazione" items="${prenotazioni}">
              <tr>
                ...
                <td>
                <span class="${prenotazione.dataScadenza>prenotazione.dataConsegna?'text-success':'text-danger'}">
                ...
                <c:if test="${prenotazione.stato=='in corso'}">
                 <form action="/biblioteca/admin/prenotazioni/chiudi/${prenotazione.id}" method="post">
                    <button type="submit" class="btn btn-sm btn-outline-primary chiudi"  name="chiudiBtn" value="0">Chiudi prenotazione</button>
                  </form>
                </c:if>
                </td>
            </tr>
